...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type'])) {
    ...
    try {
        if ($form_type == "facility") {
            $stmt = $conn->prepare("INSERT INTO facilities (name, type, capacity, status) VALUES (?, ?, ?, ?)");
            if (!$stmt) throw new Exception("SQL Error (facility): " . $conn->error);
            $stmt->bind_param("ssis", $_POST['name'], $_POST['type'], $_POST['capacity'], $_POST['status']);
            $stmt->execute();
            ...
            if (strtotime($_POST['end_time']) <= strtotime($_POST['start_time'])) {
                $_SESSION['feedback'] = ['type' => 'error', 'message' => 'End time must be after start time.'];
            } else {
                $stmt = $conn->prepare("INSERT INTO reservations (facility_id, reserved_by, purpose, status, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?)");
                if (!$stmt) throw new Exception("SQL Error (reservation): " . $conn->error);
            ...
            if (!$stmt) throw new Exception("SQL Error (maintenance): " . $conn->error);
  ...
  <main class="ml-64 flex-1 flex flex-col">
    ...
    <div class="flex-1 p-6 overflow-y-auto">
      ...
      <div id="reservations" class="tab-content hidden">
        <form method="POST" class="bg-white p-6 rounded-lg shadow space-y-4 max-w-lg" autocomplete="off" id="reservationForm">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setActiveTab(tabFromUrl || savedTab || 'facilities');
