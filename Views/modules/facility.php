...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type'])) {
    ...
    try {
        ...
        $_SESSION['feedback'] = ['type' => 'success', 'message' => ucfirst($form_type) . " submitted successfully!"];
        $stmt->close();
...
// --- FETCH FACILITIES FOR SELECT OPTIONS ---
$facilityOptions = "";
...
<main class="ml-64 flex-1 flex flex-col">
  ...
  <div class="flex-1 p-6 overflow-y-auto">
    ...
    <!-- Facility Form -->
    <div id="facilities" class="tab-content hidden">
      <form method="POST" class="bg-white p-6 rounded-lg shadow space-y-4 max-w-lg" autocomplete="off">
        <h3 class="text-lg font-semibold border-b pb-2 mb-4">New Facility</h3>
        <input type="hidden" name="form_type" value="facility" />
        <input type="text" name="name" placeholder="Facility Name" required class="w-full border rounded-md px-3 py-2" />
        ...
        <select name="status" required class="w-full border rounded-md px-3 py-2">
          <option value="Available">Available</option>
          <option value="Unavailable">Unavailable</option>
        </select>
        <textarea name="notes" placeholder="Optional notes..." class="w-full border rounded-md px-3 py-2"></textarea>
    ...
    <div id="reservations" class="tab-content hidden">
      <form method="POST" id="reservationForm" class="bg-white p-6 rounded-lg shadow space-y-4 max-w-lg" autocomplete="off">
        <h3 class="text-lg font-semibold border-b pb-2 mb-4">New Reservation</h3>
        ...
        <input type="text" name="purpose" placeholder="Purpose" required class="w-full border rounded-md px-3 py-2" />
        <label>Start Time:
    ...
    <div id="maintenance" class="tab-content hidden">
      <form method="POST" class="bg-white p-6 rounded-lg shadow space-y-4 max-w-lg" autocomplete="off">
        <h3 class="text-lg font-semibold border-b pb-2 mb-4">Maintenance Request</h3>
        ...
        <select name="priority" required class="w-full border rounded-md px-3 py-2">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function setActiveTab(tabId) {
        tabs.forEach(tab => {
            tab.className = `tab-link pb-2 ${inactiveTabClass}`;
    ...
    tabs.forEach(tab => tab.addEventListener('click', e => setActiveTab(e.target.dataset.tab)));

    const urlParams = new URLSearchParams(window.location.search);
    const initialTab = urlParams.get('tab') || localStorage.getItem('activeFormTab') || 'facilities';
