...
// NOTE: Form handling logic can be placed here.
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_type'])) {
    // Process forms for facility, reservation, maintenance...
}
// Fetch all data needed for the page display
$facilitiesResult = $conn->query("SELECT * FROM facilities ORDER BY id ASC");
if (!$facilitiesResult) {
    die("Facilities query failed: " . $conn->error);
}

$resResult = $conn->query("SELECT r.*, f.name AS facility_name FROM reservations r JOIN facilities f ON r.facility_id = f.id ORDER BY r.start_time DESC");
if (!$resResult) {
    die("Reservations query failed: " . $conn->error);
}

$mainResult = $conn->query("SELECT m.*, f.name AS facility_name FROM maintenance m JOIN facilities f ON m.facility_id = f.id ORDER BY m.created_at DESC");
if (!$mainResult) {
    die("Maintenance query failed: " . $conn->error);
}

// Now it's safe to use ->num_rows
...
function getStatusBadge($status) {
    ...
    // Handle priority values for maintenance
    if (in_array($statusLower, ['high'])) $color = 'red';
    if (in_array($statusLower, ['medium'])) $color = 'yellow';
    if (in_array($statusLower, ['low'])) $color = 'green';
    
    // Handle status values for other tables
...
<body class="bg-gray-100 flex h-screen overflow-hidden">
        ...
        <div class="flex-1 p-4 lg:p-6 overflow-y-auto">
            <div class="border-b border-gray-200 mb-6">
                <nav class="flex -mb-px space-x-6" aria-label="Tabs">
                    <button class="tab-link whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm border-blue-600 text-blue-600" data-tab="facilities">Facilities</button>
                    <button class="tab-link whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-tab="reservations">Reservations</button>
                    <button class="tab-link whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300" data-tab="maintenance">Maintenance</button>
                </nav>
            </div>
            ...
            <div id="facilities" class="tab-content">
                <div class="bg-white rounded-lg shadow overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <?php if ($facilitiesResult->num_rows > 0): mysqli_data_seek($facilitiesResult, 0); while ($row = $facilitiesResult->fetch_assoc()) : ?>
                            ...
                            <?php endwhile; else: ?>
                                <tr><td colspan="6" class="text-center py-8 text-gray-500">No facilities found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
            ...
            <div id="reservations" class="tab-content hidden">
                 <div class="bg-white rounded-lg shadow overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <?php if ($resResult->num_rows > 0): mysqli_data_seek($resResult, 0); while ($row = $resResult->fetch_assoc()) : ?>
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-6 py-4"><?= $row['id'] ?></td>
                                    <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($row['facility_name']) ?></td>
                            ...
                            <?php endwhile; else: ?>
                                <tr><td colspan="7" class="text-center py-8 text-gray-500">No reservations found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
            ...
            <div id="maintenance" class="tab-content hidden">
                 <div class="bg-white rounded-lg shadow overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <?php if ($mainResult->num_rows > 0): mysqli_data_seek($mainResult, 0); while ($row = $mainResult->fetch_assoc()) : ?>
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="px-6 py-4"><?= $row['id'] ?></td>
                                    <td class="px-6 py-4 font-medium text-gray-900"><?= htmlspecialchars($row['facility_name']) ?></td>
                            ...
                            <?php endwhile; else: ?>
                                <tr><td colspan="7" class="text-center py-8 text-gray-500">No maintenance records found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="fixed bottom-6 right-6 z-50">
        <button id="chatbotToggle" class="bg-gray-800 hover:bg-gray-700 text-white rounded-full p-2 shadow-lg transition-all duration-300 group">
            ...
            <span class="absolute bottom-full mb-2 right-1/2 transform translate-x-1/2 bg-gray-900 text-white text-xs rounded py-1 px-2 opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-lg">Admin Assistant</span>
        </button>
        <div id="chatbotBox" class="fixed bottom-24 right-6 w-80 bg-white border border-gray-200 rounded-xl shadow-xl opacity-0 scale-95 pointer-events-none transition-all duration-300 overflow-hidden">
            <div class="p-4 border-b border-gray-200 bg-blue-600 text-white"><h3 class="font-semibold">Admin Assistant</h3></div>
    ...
    class AdminChatbot {
        constructor() {
            this.elements = {
                ...
                quickBtns: document.querySelectorAll('.quickBtn')
            };
            this.state = { isOpen: false, maxHistory: 50 };
            this.responses = [
                "I can help you find information about facilities, check on reservations, or view maintenance schedules.",
                `The system shows ${<?= $totalFacilities ?>} facilities, ${<?= $totalReservations ?>} reservations, and ${<?= $totalMaintenance ?>} maintenance records.`,
                "You can view all facilities, reservations, and maintenance tasks in the tables above. What specific information are you looking for?"
            ];
            this.init();
        }
        init() {
        ...
        toggle() {
            ...
            if (this.state.isOpen) {
                this.elements.chatBox.classList.remove('opacity-0', 'scale-95', 'pointer-events-none');
                this.elements.chatBox.classList.add('opacity-100', 'scale-100', 'pointer-events-auto');
                this.elements.userInput.focus();
            } else {
                this.elements.chatBox.classList.remove('opacity-100', 'scale-100', 'pointer-events-auto');
                this.elements.chatBox.classList.add('opacity-0', 'scale-95', 'pointer-events-none');
            }
        }
        addMessage(message, isUser = false) {
            const messageWrapper = document.createElement('div');
            messageWrapper.className = `flex items-start gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`;
            
            const avatar = `<div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${isUser ? 'bg-gray-600' : 'bg-blue-600'}">${isUser ? 'U' : 'AI'}</div>`;
            const content = `<div class="p-3 rounded-lg shadow-sm max-w-xs ${isUser ? 'bg-blue-600 text-white' : 'bg-white text-gray-800'}"><p class="text-sm">${message}</p></div>`;
            
            messageWrapper.innerHTML = avatar + content;
            this.elements.chatContent.appendChild(messageWrapper);
        ...
        getAIResponse(userMessage) {
            ...
            if (lowerMessage.includes('facility')) return `We have ${<?= $totalFacilities ?>} facilities registered. You can see their status in the 'Facilities' tab.`;
            if (lowerMessage.includes('reservation')) return `There are ${<?= $totalReservations ?>} reservations logged. Check the 'Reservations' tab for details.`;
            if (lowerMessage.includes('maintenance') || lowerMessage.includes('issue')) return `We have ${<?= $totalMaintenance ?>} maintenance records. You can view them in the 'Maintenance' tab.`;
            return this.responses[Math.floor(Math.random() * this.responses.length)];
        }
        handleQuickAction(action) {
            const messages = {
                'facilities': 'Tell me about the facilities',
                'reservations': 'How many reservations are there?',
                'maintenance': 'Show me the maintenance log'
            };
            if (messages[action]) {
