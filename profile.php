...
<div class="relative inline-block text-left">
  <button id="userDropdownToggle" class="inline-flex items-center gap-2 rounded border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none cursor-pointer select-text">
    ...
    <svg id="dropdownArrow" class="w-4 h-4 text-gray-500 rotate-180 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
      <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
    </svg>
  </button>

  <!-- Dropdown Menu (VISIBLE on load) -->
  ...
  <div id="logoutModal" class="absolute right-0 mt-2 w-44 bg-white p-4 rounded-md shadow-lg hidden z-50">
    ...
    <div class="flex justify-between">
      ...
      <a href="../auth/login.php" id="confirmBtn" class="px-4 py-2 bg-red-500 text-white rounded-md">Logout</a>
    </div>
  </div>
</div>
...
<script>
  ...
  // Toggle dropdown visibility & arrow rotation on button click
  toggleBtn.addEventListener('click', (e) => {
    e.stopPropagation(); // prevent event bubbling
    dropdown.classList.toggle('hidden');
    arrow.classList.toggle('rotate-180');
    logoutModal.classList.add('hidden'); // hide modal if open
  });

  // Hide dropdown and reset arrow if clicking outside
